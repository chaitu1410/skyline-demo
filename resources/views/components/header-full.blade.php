<div>
  ...
  <!-- categories start -->
  <div id="categoriessticktop" class="nav-scroller bg-body shadow-sm">
    <nav class="nav nav-underline" aria-label="Secondary navigation">

      <a id="allcatbtn" class="nav-link categoryscollitem" href="{{ route('categories.index') }}">
        <img src="{{ asset('assets/images/all.png') }}" alt="">
        <p class="mb-0">All</p>
      </a>

      @forelse ($categories as $category)    
        <a class="nav-link categoryscollitem" href="{{ route('categories.show', $category) }}">
          <img src="{{ asset('images/'.$category->image) }}" alt="{{$category->name}}">
          <p class="mb-0">{{Str::limit($category->name, 20, $end='...')}}</p>
        </a>
      @empty
